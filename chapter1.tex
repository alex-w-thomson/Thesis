...
By the equivalences $\norm{T_\sigma(f,g)}{\besw{p}{s}{q}{w}} \sim \norm{D^s T_\sigma(f,g)}{\besw{p}{0}{q}{w}}$ and $\norm{T_\sigma(f,g)}{\tlw{p}{s}{q}{w}} \sim \norm{D^s T_\sigma(f,g)}{\tlw{p}{0}{q}{w}}$ estimates \ref{KP:CM:TL} and \ref{thm:CM:TL:B} can be seen as estimates of the form \ref{h_general_estimate}. In particular in the case that $\sigma \equiv 1$ we obtain results that both extend and improve the estimates \ref{def:leibniz} and \ref{def:ileibniz}. 
...
In Chapter 3 we present Leibniz-type rules in Besov spaces for bilinear pseudodifferential operators associated to symbols in bilinear H\"ormander classes of critical order. The main result of this chapter is the following theorem. 
...
[\citep{MR3179688}, \citep{miyachi2018bilinear}, \citep{miyachi2018bilinear2}], Naibo [\citep{MR3393696}, \citep{MR3411149}], Rodr\'ıguez-L\'opez-Staubach \citep{MR3035059}, and the references therein.
