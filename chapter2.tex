...
For $y\in\rn$ denote by $\tau_y$ the operator given by $\tau_y h(x)=h(x+y)$ for  $x\in\rn.$ 
...
Let $\Psi\in \sw$ be such that $\int_{\rn}\Psi(x)\dx\neq 0.$ Given $0<p<\infty,$ the  Hardy space $H^p(w)$ is defined as 
\begin{equation*}
H^p(w) :=\{f \in \swp: \norm{f}{H^p(w)}< \infty \},
\end{equation*}
where 
...
\]
The Sobolev space $\dot{W}^{s,p}(w)$ is the space of all tempered distributions modulo polynomials such that 
