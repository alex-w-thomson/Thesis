...
The notation $A \lesssim B$ means that $A \leq c B$, where $c$ is a constant that may depend on some of the parameters used but not on the functions involved.
...
The Leibniz-type rules obtained in two of the main results of this chapter, Theorem \ref{thm:CM:TL:B} and Theorem \ref{thm:ICM:TL:B}, hold in the setting of weighted Triebel-Lizorkin and Besov spaces. In this section we define these scales of spaces and present some of their properties. In particular, we state and prove Nikol'skij representations, which constitute important tools for the proofs of Theorems \ref{thm:CM:TL:B} and \ref{thm:ICM:TL:B}.
...
The Muckenhoupt classes arise in the study of boundedness properties of the Hardy-Littlewood maximal operator in the setting of weighted Lebesgue spaces, as we next explain. The \textit{Hardy-Littlewood maximal operator} $\mathcal{M}$ is defined as 
...
For more details on the Muckenhoupt classes see Muckenhoupt \cite{MR0293384} and Grafakos \cite{MR3243741}.
...
\end{equation*}
with appropriate changes if $q=\infty$.
\item For $0<p\leq\infty$, the \textit{weighted inhomogeneous Besov space} $\besw{p}{s}{q}{w}$ consists of all $f\in\mathcal{S}'(\rn)$ such that 
\begin{equation*}
...
\begin{enumerate}[(i)]
\item \label{TL_equal1} $\tlw{p}{0}{2}{w} = H^p (w) \text{ for } 0<p<\infty,  w\in A_\infty,  $
\item \label{TL_equal2}  $\itlw{p}{0}{2}{w} = h^p(w) \text{ for } 0<p<\infty,  w\in A_\infty,$ 
\item \label{TL_equal3}  $\tlw{p}{0}{2}{w} = \itlw{p}{0}{2}{w} = L^p(w) = H^p(w) = h^p(w) \text{ for } 1<p<\infty,  w\in A_p,$ 
\item \label{TL_equal4}  $\tlw{p}{s}{2}{w} = \dot{W}^{s,p}(w) \text{ for } 1<p<\infty,  w\in A_p$, 
\end{enumerate}
...
An important tool for the proofs of Theorem \ref{thm:CM:TL:B} and Theorem \ref{thm:ICM:TL:B} is the Nikol'skij representation for weighted Triebel-Lizorkin and Besov spaces, stated below as Theorem \ref{thm:Nikolskij:weighted}. Such result is a weighted version of \cite[Theorem 3.7]{MR837335}. 

Given $0<p,q\leq\infty$, and a sequence of functions $\{ f_j\}_{j\in\mathbb{Z}}$ defined on $\re^n$, the following notation will be used in this section:

\[ \norm{\{f_j\}_{j\in\mathbb{Z}}}{L^p(w)(\ell^q)}:= \norm{\left( \sum\limits_{j\in\mathbb{Z}} |f_j|^q \right)^{1/q} }{L^p(w)}, \]
...
The case $q = \infty$ is straightforward. This completes the proof.
...
%
%Triebel--Lizorkin and Besov spaces based in weighted Lebesgue spaces posses the Fatou property. This follows from the fact that for $0<p\leq\infty$, $L^p(w)$ satisfies: (1) if $f,g\in L^p(w)$ and $\abs{f}\le\abs{g}$ pointwise a.e., then $\norm{f}{L^p(w)}\lesssim \norm{g}{L^p(w)};$ (2) if $\{f_j\}_{j\in \na}\subset L^p(w)$ and $f_j\ge 0$ poinwise a.e., then $\norm{\liminf_{j\to\infty} f_j}{L^p(w)}\lesssim \liminf_{j\to\infty}\norm{f_j}{L^p(w)}.$ 
%\end{remark}
 \section{Leibniz-type rules in weighted Triebel-Lizorkin and Besov spaces}\label{sec:ch2_results}
 ...
 We remind the reader that the notation $\tau_{p,q}(w)$ and $\tau_p (w)$ for $w\in A_\infty$ is defined in Section \ref{sec:weighted_TL_B} see \eqref{def:tau_pq(w)}; we recall that $\tau_{p,2}(w) = \tau_p(w)$ and we write $\tau_{p,q}$ and $\tau_p$ if $w\equiv 1$. 
...
\begin{equation}\label{lift:KP:CM:TL}
\norm{D^s T_\sigma(f,g)}{\tlw{p}{0}{q}{w}} \lesssim \norm{D^s f}{\tlw{p_1}{m}{q}{w_1} } \norm{g}{H^{p_2}(w_2)} +  \norm{f}{H^{p_1}(w_1)}   \norm{D^s g}{\tlw{p_2}{m}{q}{w_2} }, 
\end{equation}

\begin{equation}\label{lift:KP:CM:B}
\norm{D^s T_\sigma(f,g)}{\besw{p}{0}{q}{w}} \lesssim \norm{D^s f}{\besw{p_1}{m}{q}{w_1} } \norm{g}{H^{p_2}(w_2)} +  \norm{f}{H^{p_1}(w_1)}   \norm{D^s g}{\besw{p_2}{m}{q}{w_2} },
\end{equation}

\begin{equation}\label{lift:KP:CM:TL2}
\norm{D^s T_\sigma(f,g)}{\tlw{p}{0}{q}{w}} \lesssim \norm{D^s f}{\tlw{p}{m}{q}{w} } \norm{g}{L^\infty} +  \norm{f}{L^\infty}   \norm{D^s g}{\tlw{p}{m}{q}{w}}.
\end{equation}
...
\begin{equation}\label{KP:CM:Hardy1}
\norm{D^s(T_\sigma(f,g))}{H^p(w)} \lesssim \norm{D^s f}{H^{p_1}(w_1)} \norm{g}{H^{p_2}(w_2)} +  \norm{f}{H^{p_1}(w_1)}   \norm{D^s g}{H^{p_2}(w_2)} \quad \forall f, g \in \swz.
\end{equation}
If $w_1=w_2$ then different pairs of $p_1, p_2$ can be used on the right-hand side of \eqref{KP:CM:Hardy1}; moreover, if $w\in A_\infty,$ then 
...
Taking $\sigma \equiv 1$ in Corollary \ref{coro:KP:CM:Hardy}, we obtain weighted fractional Leibniz rules for the product of two functions: 
...
We close this section by comparing Corollaries \ref{coro:KP:CM:Hardy} and \ref{coro:KP:Hardy} with other related results in the literature: 
...
Corollary \ref{coro:KP:CM:Hardy} and this result compare as follows: 

\begin{itemize}
\item The estimate (\ref{KP:CM:Hardy1}) allows for $0<p,p_1,p_2 < \infty$, $w_1,w_2 \in A_\infty$, and $H^p(w)$ on the left-hand side if $s>\tau_p(w)$. On the other hand, (\ref{KP:CM:Lebesgue}) requires $1<p_1,p_2\le \infty,$ $w_1\in A_{p_1},$ $w_2\in A_{p_2}$ and the smaller $L^p(w)$ norm on the left-hand side when $s>\tau_{p}$. Therefore, (\ref{KP:CM:Hardy1}) is less restrictive than (\ref{KP:CM:Lebesgue}) in terms of the indices $p$, $p_1$, and $p_2$ and the classes that the weights $w_1$ and $w_2$ belong to. However, since $\tau_{p} \leq \tau_p(w)$, (\ref{KP:CM:Hardy1}) is more restrictive than (\ref{KP:CM:Lebesgue}) in terms of the range of the regularity $s$.
\item  Let  $1/2<p<\infty,$ $1<p_1,p_2<\infty$ such that $\hcline,$  $w_1\in A_{p_1}$ and $w_2\in A_{p_2}$. If $s>\tau_{p}(w)$ then \eqref{KP:CM:Hardy1} holds and implies \eqref{KP:CM:Lebesgue}; however, if $\tau_p < \tau_p(w)$ then \eqref{KP:CM:Lebesgue} holds also for $\tau_p < s \leq \tau_p(w)$ while (\ref{KP:CM:Hardy1}) may not be true for such range of $s$. We next give examples of $w_1$ and $w_2$ such that the corresponding weight $w$ satisfies $\tau_p < \tau_p(w)$.  Let $1<p_1\leq p_2 <\infty$ and $w_1(x) = w_2(x) = w(x) = |x|^a$ with $n(r-1)<a<n(p_1-1)$ for some $1<r<p_1$. Then $w \in A_{p_1} \subset A_{p_2}$, and $w \notin A_r$. This implies that $1<\tau_w$ which leads to $\tau_p < \tau_p(w)$ if $p<\tau_w$. 
\item  The estimate (\ref{KP:CM:Lebesgue}) implies (\ref{Kp:CM:Hardy2}) for $1<p<\infty$, $w\in A_p$, and $s>\tau_{p}$ and gives the endpoint estimate 
$$ \norm{D^s(T_\sigma(f,g))}{L^p(w)} \lesssim \norm{D^s f}{L^{\infty}} \norm{g}{L^{p}(w)} +  \norm{f}{L^{p}(w)}\norm{D^s g}{L^\infty}. $$
\noindent However, (\ref{Kp:CM:Hardy2}) allows $0<p<\infty$ and $w\in A_\infty$ if $s>\tau_p (w)$.
\end{itemize}




Corollary~\ref{coro:KP:Hardy} complements some of the estimates obtained through different methods in \cite[Theorem 1.1]{MR3513582} in the same manner  Corollary~\ref{coro:KP:CM:Hardy} complements  \cite[Theorem 1.1]{BrNa2017} as explained above; as in that case, 
...
In this section we prove Theorem \ref{thm:CM:TL:B}. The following lemma will be useful in the proof of Theorem \ref{thm:CM:TL:B}.
...
For ease of notation, $p_1$ and $p_2$ will be assumed to be finite; the same proof applies for \eqref{KP:CM:B} if that is not the case.
...
By the lifting properties for weighted inhomogeneous Besov and Triebel-Lizorkin spaces (\ref{lifting_TL}) and (\ref{lifting_B}), the estimates (\ref{KP:ICM:TL}), (\ref{KP:ICM:B}), and (\ref{KP:ICM:TL2}) can be written respectively as 
...
Corollary~\ref{coro:KP:CM:Hardyloc} applied to the case $\sigma\equiv 1$ gives in particular 
...
\begin{remark}
An analogous observation to Remark \ref{remark:thm:CM:TL:B} follows for the multiplier $\sigma$ in Theorem~\ref{thm:ICM:TL:B} in relation to the condition  obtained from \eqref{eq:CMm} with $\abs{\xi}+\abs{\eta}$ replaced by $1+\abs{\xi}+\abs{\eta}.$ 
\end{remark}
 ...
 The method used to prove Theorem (\ref{thm:CM:TL:B}) is quite versatile and can be applied to Triebel-Lizorkin and Besov spaces that are based in other quasi-Banach spaces.
...
Given $s\in\re,$ $0<q\le \infty$ and $\pp\in\mathcal{P}_0,$ the corresponding inhomogeneous Triebel-Lizorkin and Besov spaces are denoted by $\itl{\pp}{s}{q}$  and  $\ibes{\pp}{s}{q},$
...
We next consider properties \ref{item:first}-\ref{item:last} in the variable-exponent setting. Given $\pp\in \mathcal{P}_0,$ $0<q\le\infty$ and $s\in \re,$  property \ref{item:first} for $\itl{\pp}{s}{q}$ and $\ibes{\pp}{s}{q}$ with $r=\min(p_-,q,1)$ follows right away using \eqref{eq:power}.  Property \ref{item:second} is given by the following version of H\"older's inequality  in the context of variable-exponent Lebesgue spaces: If $\ppo,\, \ppt,\, \pp \in \mathcal{P}_0$ are such that
...
\end{equation*}
Finally, the following version of the Nikol'skij representation for $\itl{\pp}{s}{q}$ and $\ibes{\pp}{s}{q}$ (property \ref{item:last}), can be proved along the lines of the proof of Theorem~\ref{thm:Nikolskij:weighted} (see also Remark~\ref{appendix}):
...
We next state a version of Theorem~\ref{thm:ICM:TL:B} for variable-exponent Triebel-Lizorkin spaces as a model result.
...
%
%% \subsection{Leibniz-type rules in the setting of weighted Lorentz-based Triebel-Lizorkin and Besov spaces}
...
%%\end{equation} 
%%
%%Let $\B$ be the family of all $\pp \in \mathcal{P}_0$ such that $\M,$  the Hardy--Littlewood maximal operator, is bounded from $\Lp$ to $\Lp.$ Such exponents satisfy $p_- > 1$ and the following log-H\"older continuity properties
%%\begin{itemize}
...
%%
%%To prove \ref{item:second} we use \cite[Corollary 2.28]{MR3026953} and \ref{eq:power} to get the following version of H\"older's inequality: If $\ppo,\, \ppt,\, \pp \in \mathcal{P}_0$ are such that
%%$ {1}/{\pp} = {1}/{\ppo} + {1}/{\ppt}$ then
%%\begin{equation*}
%% \|fg\|_{L^\pp} \lesssim \|f\|_{L^\ppo}\|g\|_{L^\ppt}\quad \forall f\in L^\ppo, g\in L^\ppt.
%% \end{equation*}
%%The case when $\pp \in \mathcal{P}_0$ has $p_- \geq 1$ is shown in \cite[Corollary 2.28]{MR3026953}. If $0<p_-<1$ then we use \ref{eq:power} to get 
%%\begin{align*}
...
%%If $\pp\in\mathcal{P}_0^*,$  $0<q\le \infty$  and $0<r<\min(\tau_{\pp},q)$ then
%%\begin{equation*}
%%\norm{\left(\sum_{j\in\ent} \abs{\M_r(f_j)}^q\right)^{\frac{1}{q}}}{\Lp}\lesssim \norm{\left(\sum_{j\in\na_0} \abs{f_j}^q\right)^{\frac{1}{q}}}{\Lp}\quad \forall \{f_j\}_{j\in \na_0}\in \Lp(\ell^q);
%%\end{equation*}
%%in particular, if $0<r<\tau_{\pp}$ it holds that 
...
\subsection{Nikol'skij representations of Triebel-Lizorkin and Besov spaces}\label{appendix}
In this section we remark on the proofs of the corresponding versions of Theorem \ref{thm:Nikolskij:weighted} in the settings of Lorentz spaces, Morrey spaces, and variable-exponent Lebesgue spaces. 
...
\end{equation}
therefore, $\sum_{j\in\naz} u_j$ converges in $\itlw{(p,t)}{s}{q}{w}$ and  $\itl{\pp}{s}{q},$  respectively. In the Lorentz space $\lebw{p,t}$ the fact (\ref{eq:cauchyseq}) is  a consequence of the following dominated convergence type thoerem.

\begin{theorem}
Suppose $f_n \rightarrow f$ in measure and $|f_n (x)| \leq |g(x)|$ for some $g\in L^{p,t}(w)$. Then $$\lim_{n\rightarrow\infty} \norm{f_n-f}{L^{p,t}(w)} = 0.$$
\end{theorem}

In the setting of variable-exponent Lebesgue spaces (\ref{eq:cauchyseq}) is  a consequence of the following dominated convergence type thoerem.
\begin{theorem}
Suppose $f_n \rightarrow f$ pointwise a.e. and $|f_n (x)| \leq |g(x)|$ for some $g\in \Lp.$. Then $f_n \rightarrow f$ in $\Lp.$
\end{theorem}
...
\left\{ \begin{array}{lll}  \partial_t u =vw, & \mathcal{P}artial_t v +a(D) v = 0, & \mathcal{P}artial_t w + b(D) w = 0, \\
  u(0,x)=0,&v(0,x)=f(x),&w(0,x) = g(x),
 \end{array} \right.
\end{equation}

\noindent where $u=u(t,x)$, $v=v(t,x)$, and $w=w(t,x)$, $t\geq 0$ and $x\in\re^n$. Here the operators $a(D)$ and $b(D)$ are \textit{linear} Fourier multiplier operators with the symbols $a(\xi)$ and $b(\xi)$ respectively; that is, $\widehat{a(D)f}(\xi) = a(\xi)\widehat{f}(\xi)$ and $\widehat{b(D)f}(\xi) = b(\xi)\widehat{f}(\xi)$. 

Without taking issues of convergence into account, we get that 
\begin{equation}\label{V}
v(t,x) = \int_\rn e^{-ta(\xi)} \widehat{f}(\xi)e^{2\mathcal{P}i i x\cdot \xi} d\xi.
\end{equation}
...
&=0,
\end{align*}
where the Fourier transforms $\widehat{\partial_t v(t,\cdot)}$ and $\widehat{v(t,\cdot)}$; 
...
Indeed, we write
\begin{align*}
u(t,x) & = \int_{\rtn} \frac{1-e^{-t(|\xi|^\gamma + |\eta|^\gamma)}}{|\xi|^\gamma + |\eta|^\gamma} \widehat{f}(\xi) \widehat{g}(\eta) \, \eixxe \dxi\deta \\
	& = \int_{|\xi|,|\eta| \leq 1} \frac{1-e^{-t(|\xi|^\gamma + |\eta|^\gamma)}}{|\xi|^\gamma + |\eta|^\gamma} \widehat{f}(\xi) \widehat{g}(\eta) \, \eixxe \dxi\deta  \\
	&+ \int_{|\xi|,|\eta| > 1} \frac{1-e^{-t(|\xi|^\gamma + |\eta|^\gamma)}}{|\xi|^\gamma + |\eta|^\gamma} \widehat{f}(\xi) \widehat{g}(\eta) \, \eixxe \dxi\deta \\
	& = I + II
\end{align*}

It is easily seen that $II < \infty$. For $I$ using that $f\in\mathcal{S}_0$ we have  
\begin{align*}
\widehat{f}(\xi) & = \left\lfloor\gamma\right\rfloor
\frac{\partial^\alpha \widehat{f}(0)}{\alpha !}\xi^\alpha
 + \sum\limits{|\alpha| = 
 \left\lfloor\gamma\right\rfloor + 1} \frac{\partial^\alpha \widehat{f}(c \xi)}{\alpha !}\xi^\alpha \\
 & = \sum_{|\alpha| = 
 \left\lfloor\gamma\right\rfloor + 1} \frac{\partial^\alpha \widehat{f}(c \xi)}{\alpha !}\xi^\alpha,
\end{align*}
for some $0<c<1$. Because $|\xi|\leq 1$ we bound $\partial^\alpha \widehat{f}(c \xi)$ uniformly in $c$ and $\xi$, $\max |\partial^\alpha \widehat{f}(c \xi)| \leq \max\limits_{|\xi|\leq 1} \partial^\alpha \widehat{f}(\xi) < \mathcal{C}$ for some $\mathcal{C}<\infty$. Therefore,
$$ |I| \leq \int_{|\xi|,|\eta| \leq 1} \mathcal{C} |\xi|^{1 + \left\lfloor\gamma\right\rfloor - \gamma }  \widehat{g}(\eta) \, \eixxe \dxi\deta < \infty.$$


So by applying the Dominated Convergence Theorem and letting $t \rightarrow \infty$ we get that $u(t,\cdot)$ converges to $u_\infty$ both pointwise and in $\mathcal{S}'$. 
